...
sumsOf :: [Int] -> [Int]
sumsOf []     = []
sumsOf (n:ns) = n : addToAll n (sumsOf ns)
    where
        addToAll :: Int -> [Int] -> [Int]
        addToAll i []     = []
        addToAll i (j:ns) = (i + j) : addToAll i ns
...
mymaximum bigger initial (a:as) = if bigger a maxTail then a else maxTail
    where
        maxTail = mymaximum bigger initial as
------------------------------------------------------------------------------

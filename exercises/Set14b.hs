...
balance db account = do
   result <- query db balanceQuery [account]
...
data Command = Deposit  T.Text Int 
             | Withdraw T.Text Int
             | Balance  T.Text 
             deriving (Show, Eq)
...
parseCommand (t:ts) | t == T.pack "balance"  = parseBalanceCommand  ts 
                    | t == T.pack "deposit"  = parseDepositCommand  ts 
                    | t == T.pack "withdraw" = parseWithdrawCommand ts 
                    | otherwise              = Nothing 
  where
    parseBalanceCommand [acc] = Just $ Balance acc
    parseBalanceCommand    _  = Nothing

    parseDepositCommand  [acc,amt] = parseInt amt >>= \n -> Just $ Deposit acc n
    parseDepositCommand        _  = Nothing

    parseWithdrawCommand [acc,amt] = parseInt amt >>= \n -> Just $ Withdraw acc n
    parseWithdrawCommand        _  = Nothing
...
perform db (Just (Deposit  acc amt)) = deposit db acc   amt  >> return (T.pack "OK")
perform db (Just (Withdraw acc amt)) = deposit db acc (-amt) >> return (T.pack "OK")
...
server db request respond = do
  msg <- perform db (parseCommand (pathInfo request))
  respond (responseLBS status200 [] (encodeResponse msg))

...
combine f (Picture pic1) (Picture pic2) = Picture (\p -> f (pic1 p) (pic2 p))
...
paintSolid c (Shape s) (Picture f) = Picture (\p -> if s p then c else f p)
...
paint (Picture p) (Shape s) (Picture b) = Picture (\px -> if s px then p px else b px)
...
instance Transform Blur where
  apply Blur (Picture f) = Picture ptToAvgColor
    where
      ptToAvgColor = avg . map f . neighbors

      avg colors = Color avgRed avgGreen avgBlue
        where
          avgRed   = sum (map getRed   colors) `div` length colors
          avgGreen = sum (map getGreen colors) `div` length colors
          avgBlue  = sum (map getBlue  colors) `div` length colors

      neighbors (Coord x y) =  [ Coord  x     y
                               , Coord (x+1)  y
                               , Coord (x-1)  y
                               , Coord  x    (y+1)
                               , Coord  x    (y-1)
...
instance Transform BlurMany where
   apply (BlurMany n) pict = iterate (apply Blur) pict !! n

...
longest :: (Ord a) => [[a]] -> [a]
longest as = go (head as) as
    where
        go bs [] = bs

        go bs (as:rest) | isBetter as bs = go as rest
                          | otherwise    = go bs rest

        isBetter xs ys = length xs > length ys ||
            (length xs == length ys && head xs < head ys)
...
incrementKey _ [] = []
incrementKey k ((k',v):kvs) | k == k'   = (k', 1 + v) : incrementKey k kvs
                            | otherwise = (k',     v) : incrementKey k kvs
...
winner scores player1 player2 | score1 < score2 = player2
                              | otherwise       = player1
    where
...
freqs = foldr (Map.alter add1) Map.empty
    where
        add1 Nothing  = Just 1
        add1 (Just n) = Just (n + 1)
...
transfer from to amount bank =
    if
        amount >= 0 && hasAccount from && hasAccount to && fromBal >= amount
    then
        Map.insert to (toBal + amount) $ Map.insert from (fromBal - amount) bank
    else
        bank

    where
        hasAccount s = Map.member s bank

        fromBal = Map.findWithDefault 0 from bank
        toBal   = Map.findWithDefault 0   to bank
...
swap i j arr = arr // [(i, val_j), (j, val_i)]
    where
        val_i = arr ! i
        val_j = arr ! j
...
maxIndex arr = fst $ foldr maxTup (head pairs) pairs
    where
        pairs = Data.Array.assocs arr

        maxTup (a,b) (c,d) | b < d     = (c,d)
                           | otherwise = (a,b)

...
instance Functor TwoList where
  ...
  fmap f (TwoNode a1 a2 as) = TwoNode (f a1) (f a2) $ fmap f as
...
inBoth fa fb = toList fa `intersect` toList fb
...
instance Foldable List where
  foldr _ b  Empty       = b
  foldr f b (LNode a as) = f a $ foldr f b as
...
instance Foldable TwoList where
  ...
  foldr f b (TwoNode a1 a2 as) = f a1 $ f a2 $ foldr f b as
...
sumTree :: Monoid m => Tree m -> m
sumTree  Leaf          = mempty
sumTree (Node a n1 n2) = sumTree n1 <> a <> sumTree n2

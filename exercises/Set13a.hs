...
checkNumber (s1, s2) =
  if noDigits s1 && noDigits s2 then Just (s1, s2)
                                else Nothing
  where
    noDigits = null . intersect "0123456789"
...
checkCapitals (s1, s2) =
  if caps s1 && caps s2 then Just (s1, s2)
                        else Nothing
  where
    caps s = not (null s) && isUpper (head s)
...
winner scores player1 player2 = do
  ...
  return $ if score1 < score2 then player2 else player1
...
selectSum xs is = go is 0
  where
    go     [] sum = Just sum
    go (i:is) sum = safeLookup xs i >>= \s -> go is (sum + s)

    safeLookup xs i = if i >= 0 && i < length xs then Just $ xs !! i else Nothing

selectSum2 :: Num a => [a] -> [Int] -> Maybe a
selectSum2 xs     [] = Just 0
selectSum2 xs (i:is) = do
  s   <- safeLookup xs i
  sum <- selectSum2 xs is
  return $ s + sum
...
countAndLog p = foldl' updateCount (return 0)
  where
    updateCount logger n = do
      currCount <- logger
      if p n then do
        msg $ show n
        return $ currCount + 1
...
balance accountName = BankOp f
  where
    f (Bank map) = case Map.lookup accountName map of
      Nothing -> (0, Bank map)
      Just n  -> (n, Bank map)
...
rob from to = balance from +> \bal -> withdrawOp from bal +>> depositOp to bal
...
paren '(' = do
  n <- get
  if n < 0 then return () else modify (+1)

paren ')' = do
  n <- get
  if n < 0 then return () else modify (subtract 1)
...
count a = modify f
  where
    f map  = case lookup a map of
      Nothing -> (a,     1) :                      map
      Just n  -> (a, n + 1) : filter ((/=a) . fst) map

...
velocity (Distance d) (Time s) = Velocity (d / s)

-- travel computes a distance given a velocity and a time
travel :: Velocity -> Time -> Distance
travel (Velocity v) (Time s) = Distance (v * s)
...
add v (Set s) = Set $ go v s
  where
    go v []                = [v]
    go v as | v == head as = as
            | v <  head as = v:as
            | otherwise    = head as : go v (tail as)
...
reverseNonEmpty (a :| []) = a :| []
reverseNonEmpty (a :| as) = last as :| reverse (a : init as)
...
passwordAllowed pw (ContainsSome   cs) = not $ passwordAllowed pw (DoesNotContain cs)
passwordAllowed pw (DoesNotContain cs) = null (pw `intersect` cs)
passwordAllowed pw (And pwReq1 pwReq2) = passwordAllowed pw pwReq1 && passwordAllowed pw pwReq2
passwordAllowed pw (Or  pwReq1 pwReq2) = passwordAllowed pw pwReq1 || passwordAllowed pw pwReq2
